...
class SessionDetailScreen extends StatelessWidget {
  ...
  // ========== ЦВЕТОВАЯ ПАЛИТРА В МИНИМАЛИСТИЧНОМ СТИЛЕ ==========
  static const Color _backgroundColor = Color(0xFF0A0A0A); // Почти черный фон
  static const Color _surfaceColor = Color(0xFF1A1A1A); // Поверхности карточек
  static const Color _primaryColor = Color(0xFF00FF9D); // Яркий салатово-зелёный (акцент)
  ...
  Widget build(BuildContext context) {
    // ❗️ИСПРАВЛЕНО: используем _calculateMapBoundsAndZoom из этого файла
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // 1. КАРТА В КОНТЕЙНЕРЕ БЕЗ РАМОК
          Expanded(
            flex: 2,
            child: ClipRRect(
              ...
              child: FlutterMap(
                ...
                children: [
                  // Стилизованная тёмная карта
                  TileLayer(
                    urlTemplate:
                        'https://tile.openstreetmap.org/{z}/{x}/{y}.png', // ❗️ИСПРАВЛЕНО: убран лишний пробел
          ...
          // 2. КОМПАКТНАЯ ПАНЕЛЬ СТАТИСТИКИ В МИНИМАЛИСТИЧНОМ СТИЛЕ
          _buildMinimalStatsPanel(),

          // 3. ДОСТОПРИМЕЧАТЕЛЬНОСТИ И ФАКТЫ (ТОЛЬКО ЕСЛИ ЕСТЬ)
          if (session.spokenFactIndices.isNotEmpty ||
              _getNearbyLandmarks(session.route).isNotEmpty)
            _buildMinimalPoiSection(session),

          // 4. КНОПКИ ДЕЙСТВИЙ С ГЕОМЕТРИЙ
          Padding(
            padding: const EdgeInsets.fromLTRB(24, 16, 24, 32),
            child: Row(
              children: [
                // Кнопка "Повторить" - контурная
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: () => Navigator.pop(context),
                    icon: const Icon(Icons.replay, size: 20),
                    label: const Text('Повторить маршрут',
  ...
  // 2. МИНИМАЛИСТИЧНАЯ ПАНЕЛЬ СТАТИСТИКИ
  Widget _buildMinimalStatsPanel() {
    ...
    return Container(
      ...
      child: Column(
        children: [
          // Основная строка: дистанция и время крупно
          Row(
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Text('${session.distance.toStringAsFixed(2)} км',
          ...
          // Вторая строка: темп, калории, факты
          Container(
            padding: const EdgeInsets.symmetric(vertical: 16),
            decoration: BoxDecoration(
              border: Border(
            ...
            child: Row(
              ...
              children: [
                ...
                _buildMinimalMetric(
                  Icons.menu_book,
                  'Фактов', // ❗️ИСПРАВЛЕНО: текст метки
                  session.spokenFactIndices.length.toString(), // ❗️ИСПРАВЛЕНО: используем длину списка
                ),
              ],
            ),
          ),
  ...
  // 3. МИНИМАЛИСТИЧНАЯ СЕКЦИЯ ДОСТОПРИМЕЧАТЕЛЬНОСТЕЙ
          ...
          if (session.spokenFactIndices.isNotEmpty) ...[
            Row(
              children: [
                ...
                const SizedBox(width: 8),
                Text('Интересные факты (${session.spokenFactIndices.length})', // ❗️ИСПРАВЛЕНО: показываем количество
                    style: TextStyle(
                        fontSize: 16,
                        color: _textPrimary,
                        fontWeight: FontWeight.w500)),
            ...
            const SizedBox(height: 12),
            // ❗️ИСПРАВЛЕНО: УБРАНО .take(2), ДОБАВЛЕНА ПРОВЕРКА ИНДЕКСА
            ...session.spokenFactIndices.map((index) {
              // ДОБАВЛЯЕМ ПРОВЕРКУ ИНДЕКСА
              String factText;
              if (index >= 0 && index < kGeneralFacts.length) {
                factText = kGeneralFacts[index];
              } else {
                factText = "Интересный факт #$index"; // Запасной вариант
              }
              ...
              return Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        factText, // ❗️ИСПРАВЛЕНО: используем проверенный текст
                        style: TextStyle(
                            color: _textSecondary,
                            fontSize: 14,
                            height: 1.4),
  ...
  // 4. Метод для получения достопримечательностей
  ...
  // 5. МИНИМАЛИСТИЧНЫЕ ПОЛИЛИНИИ (монохромный градиент)
  List<Polyline> _buildMinimalPolylines(List<RoutePoint> route) {
    ...
    for (int i = 1; i < route.length; i++) {
      ...
      // Плавный градиент от светлого к акцентному в зависимости от скорости
  ...
  // 6. Шеринг и подтверждение удаления (в минималистичном стиле)
...
Калории: $calories
Факты: ${session.spokenFactIndices.length} // ❗️ИСПРАВЛЕНО: используем длину списка
